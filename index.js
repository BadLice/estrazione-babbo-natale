...
const extractWithConditions = (avaliableValuesArray, conditionCallback, lastExtractionCallback) => {
...
do {
  ...
  for (let i = people.length - 1; i >= 0; i--) {
    ...
    advicerId = extractWithConditions(
      people.filter(({ isExtracted, id }) => !isExtracted.advicer && id !== people[i].id && id !== kidId),
    ...
    if (DEBUG) {
      if (advicerId && kidId) {
        console.log(`${i} - ${people[i].name} kid-> ${people[kidId].name}\tadvicer-> ${people[advicerId].name}`);
      } else {
  ...
  result.forEach(({ kid, id }) => {
    if (people[id].exceptions.indexOf(kid) !== -1) {
      console.log(`kid is an exception: id:${id} - kid:${kid} - exceptions:${people[id].exceptions}`);
  ...
  errors.isSomeonNotExtracted = people.filter(({ isExtracted }) => !isExtracted.advicer || !isExtracted.kid).length !== 0;
